...
describe('CancelReservationUseCase', () => {
  ...
  it('should return NotFoundError when reservation does not exist', async () => {
    const useCase = new CancelReservationUseCase(makeMockRepository(null))
  ...
  it('should reject cancelling an already completed reservation', async () => {
    const repo = makeMockRepository(makeReservation('completed'))
    const useCase = new CancelReservationUseCase(repo)
